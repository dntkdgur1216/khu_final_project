...
#SBATCH --job-name=train_lf-cbm_remoteclip      # 작업 이름
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-gpu=8
#SBATCH --mem-per-gpu=29G
#SBATCH -p batch_ce_ugrad

#SBATCH -t 1-0                                  # 시간 제한: 1일
...
# [핵심 수정] 모든 인자를 한 줄로 합쳐서 줄 바꿈 오류를 방지합니다.
conda run -n lf_remote_env python train_cbm.py --dataset eurosat_rgb --backbone "remote_clip_vit_b_32" --clip_name 'ViT-B-32' --remote_clip_path "../checkpoints/models--chendelong--RemoteCLIP/snapshots/bf1d8a3ccf2ddbf7c875705e46373bfe542bce38/RemoteCLIP-ViT-B-32.pt" --concept_set "data/concept_sets/eurosat_concepts4.txt" --activation_dir "saved_activations_remoteclip" --save_dir "saved_models_remoteclip" --device cuda
